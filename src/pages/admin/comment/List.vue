    ...
    <div class="dashboard-card">
      <div class="card-header-custom">
        <div class="header-left">
          ...
          <div>
            <h2 class="page-title">Danh Sách Bình Luận</h2>
            <p class="page-subtitle">Kiểm duyệt và quản lý phản hồi của khách hàng</p>
          </div>
        </div>

        <RouterLink to="/admin/commentAdd" class="btn-primary-custom">
          <i class="bi bi-plus-lg me-2"></i> Thêm Bình Luận
        </RouterLink>
      </div>
      ...
      <div class="table-wrapper">
        <table class="modern-table">
          ...
          <tbody>
            ...
            <tr v-else-if="allComments.length === 0">
              <td colspan="6" class="text-center empty-state py-5">
                <i class="bi bi-inbox fs-1 text-muted mb-3 d-block"></i>
                <p>Chưa có bình luận nào.</p>
              </td>
            </tr>
            <tr v-else v-for="(comment, index) in allComments" :key="comment.id">
              <td class="fw-medium text-muted">{{ index + 1 }}</td>
              
              <td>
                <div class="product-info">
                  <div class="product-details">
                    <div class="product-name" style="font-size: 0.9rem;">{{ getUserName(comment.user_id) }}</div>
                  </div>
                </div>
              </td>
              ...
              <td><span class="category-pill">{{ comment.blog_title }}</span></td>
              ...
              <td>
                <div class="action-buttons">
                  <button
                    ...
                    @click="handleDelete(comment.blog_id, comment.id)"
                  >
...
const fetchAll = async () => {
    ...
    try {
        ...
        // Sort newest first
        comments.sort((a, b) => new Date(b.created_at.split('/').reverse().join('-')) - new Date(a.created_at.split('/').reverse().join('-')));
        allComments.value = comments;
    } catch (error) {
        console.error(error);
        alert('Lỗi tải danh sách bình luận');
    } finally {
        loading.value = false;
    }
};
...
const handleDelete = async (blogId, commentId) => {
    if (confirm("Bạn có chắc chắn muốn xoá bình luận này không?")) {
        const blog = blogs.value.find(b => String(b.id) === String(blogId));
        if (!blog) return;
        
        blog.comments = blog.comments.filter(c => String(c.id) !== String(commentId));
