...
const CLOUDINARY_URL = "https://api.cloudinary.com/v1_1/doi9js59k/image/upload";
const UPLOAD_PRESET = "demonew";
...
const startUpload = async () => {
  ...
  const uploadPromises = files.value.map((fileItem) =>
...
<template>
  <div class="container my-5">
    <div
      class="drop-zone border border-dashed p-5 text-center mb-4"
      :class="{ 'border-primary bg-light-subtle': isDragging }"
    ...
    >
      ...
      <p class="small text-muted">
        (Chọn ảnh với định dạng JPG/PNG)
      </p>
    ...
    <h5 v-if="files.length > 0" class="mb-3">
      Hình ảnh đã chọn ({{ files.length }}):
    </h5>
    ...
    <div class="row g-3">
      ...
      >
        <div class="position-relative shadow-sm rounded overflow-hidden">
          <img
            :src="file.previewUrl"
            class="img-fluid w-100 h-100 object-fit-cover img-thumbnail"
            :alt="file.name"
          />
